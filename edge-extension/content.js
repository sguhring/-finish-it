// Finish IT bridge
//
// Three jobs on game.scoliadarts.com:
//   1. read both players' scores out of the DOM and push them to the local app
//      (so the app never has to OCR the screen),
//   2. draw the checkout suggestions back onto the game page, so you never have
//      to look at a second window, and
//   3. tell the app when a leg ends, so the LED ring pulses.
//
...
(function () {
  ...
  const VERSION       = "v4 (pinned selector + leg end)";
  ...
  //
  // Scolia's remaining-score element is span.styles_counter__ZHHHQ. The trailing
  // hash is generated by CSS Modules and changes whenever they rebuild their
  // frontend, so we match the stable prefix instead of the full class name.
  //
  // Pinning this beats the font-size heuristic twice over: it cannot pick up a
  // statistics number by mistake, and when the counters disappear at the end of
  // a game readScores() returns nothing at all -- previously the heuristic
  // latched onto leftover stats and pushed them as if they were live scores.
  ...
  // -- Leg-Ende --------------------------------------------------------------
  // Ein Leg endet genau dann, wenn ein Restscore 0 erreicht. Das ist eine
  // Flanke, keine Dauerlage: die 0 steht nur ein paar Sekunden, danach raeumt
  // Scolia die Counter ab. Deshalb wird einmal gefeuert und dann gesperrt, bis
  // wieder ein richtiger Restscore dasteht -- sonst loeste jeder Tick waehrend
  // der 0 einen neuen Puls aus.
  //
  // Bewusst NICHT an div.styles_isFinished__uTh81 ("Finished") gehaengt: das
  // erscheint schon nach jeder Aufnahme, zusammen mit "Removing darts...",
  // also mehrmals pro Leg.
  let legArmed = true;

  async function fireLegEnd() {
    console.log("[Finish IT] Leg beendet - Puls ausgeloest");
    try {
      await fetch(APP + "/api/leg_end", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    "{}",
      });
    } catch (e) {
      legArmed = true;         // App war nicht da - beim naechsten Tick erneut
    }
  }
  ...
  async function tick() {
    ...
    const zero = s.field1 === 0 || s.field2 === 0;
    if (zero && legArmed) {
      legArmed = false;
      fireLegEnd();
    } else if (!zero && (s.field1 > 2 || s.field2 > 2)) {
      legArmed = true;         // neues Leg laeuft, wieder scharf
    }
